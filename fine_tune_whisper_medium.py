...
visualization_callback = TrainingVisualizationCallback(output_dir="./medium/whisper-lux-visuals")
...
tokenizer = WhisperTokenizer.from_pretrained("openai/whisper-medium", use_fast=False)

# Reinitialize the processor using the updated tokenizer
processor = WhisperProcessor(tokenizer=tokenizer, feature_extractor=WhisperProcessor.from_pretrained("openai/whisper-medium").feature_extractor)
...
training_args = Seq2SeqTrainingArguments(
    output_dir="./medium/whisper-lux",              # Output directory for logs and model checkpoints
    ...
    logging_dir="./medium/logs",                    # Directory for logging
...
#trainer.train(resume_from_checkpoint="./medium/whisper-lux/checkpoint-5000")
trainer.train()
...
model.save_pretrained("./medium/whisper-lux-final")
processor.save_pretrained("./medium/whisper-lux-final")
# Save the generation configuration explicitly
model.generation_config.save_pretrained("./medium/whisper-lux-final")